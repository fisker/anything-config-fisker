...
function isInDirectory(directory, file) {
  return path.normalize(file).startsWith(path.normalize(directory));
}
...
function fileParser(file, directory) {
  ...
  if (!isInDirectory(directory, source) || !fs.existsSync(source)) {
    return null;
  }
...
const packageDependencies = {
  any: _objectSpread({}, dependencies, developmentDependencies),
  dependencies,
